...
void KBinary::Wrap(LPCSTR ps, UINT_PTR len, int iVer)
{
	...
	if (len > 0)
	{
		m_len = len;
		m_capa = m_len;
		m_p = const_cast<CHAR*>(ps);//Alloc(m_len);//new CHAR[m_len];//문자열이 아니고 byte이므로 CHAR 로 할당한다.
		//memcpy(m_p, ps, m_len);
...
{
	ASSERT(0 <= len && len < 500000000); // 500 Mega max
	if (0 < len)
		return new CHAR[len];
	return NULL;
}
...
//		UINT_PTR len1 = (( (len+1) /4) +1) * 4;
	...
	else // 줄어든 경우 free 하지 않고 논리적 크기만 줄인다.
	{
		//m_p[0] = '\0';
		m_p[len] = '\0';//경계밖 끝... not null terminated data인 경우
		m_p[len + 1] = '\0';//경계밖 끝... not null terminated data인 경우
...
CStringW& KBinary::GetStr(CStringW& sBuf)
{
...
bool KBinary::IsSame(KBinary* pbin, int iOp)
{
...
//void CArchiveBase::WriteCount(DWORD_PTR dwCount)
//{
